...
const ArtistPage = () => {
    ...
    const fetchArtistData = async () => {
        try {
            const response = await fetch(`https://music-app-luca.azurewebsites.net/api/artists/getOne/${id}/${user.uid}`);
            if (!response.ok) {
    ...
    const fetchAlbumsByArtist = async () => {
        try {
            const response = await fetch(`https://music-app-luca.azurewebsites.net/api/albums/getAlbumsByArtist/${encodeURIComponent(artistData.name)}/${user.uid}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
    ...
    const handleDeleteAlbum = async (albumId) => {
        try {
            const response = await fetch(`https://music-app-luca.azurewebsites.net/api/albums/deleteOne/${albumId}/${user.uid}`, {
                method: 'DELETE'
            });
            if (!response.ok) {
                throw new Error('Network response was not ok');
