...
const ArtistPage = () => {
    ...
    const fetchArtistData = async () => {
        try {
            const response = await fetch(`https://us-central1-homework3-project.cloudfunctions.net/gcp-func-novus/api/artists/getOne/${id}/${user.uid}`);
            if (!response.ok) {
    ...
    const fetchAlbumsByArtist = async () => {
        try {
            const response = await fetch(`https://us-central1-homework3-project.cloudfunctions.net/gcp-func-novus/api/albums/getAlbumsByArtist/${encodeURIComponent(artistData.name)}/${user.uid}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
    ...
    const handleDeleteAlbum = async (albumId) => {
        try {
            const response = await fetch(`https://us-central1-homework3-project.cloudfunctions.net/gcp-func-novus/api/albums/deleteOne/${albumId}/${user.uid}`, {
                method: 'DELETE'
            });
            if (!response.ok) {
                throw new Error('Network response was not ok');
