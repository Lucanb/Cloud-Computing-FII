...
const MusicPage = () => {
    ...
    const fetchAlbumData = async () => {
        try {
            // console.log('id :',id)
            const response = await fetch(`https://my-music-app.azurewebsites.net/api/albums/getOne/${id}/${user.uid}`);
            if (!response.ok) {
    ...
    const fetchSongsByAlbum = async () => {
        try {
            const response = await fetch(`https://my-music-app.azurewebsites.net/api/songs/getSongsByAlbum/${encodeURIComponent(album.title)}/${user.uid}`);
